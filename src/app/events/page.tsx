...
import {
  Plus, Pencil, Trash2, ArrowLeft, CalendarDays,
  X, ImageOff, Loader2, AlertCircle, Search, UploadCloud, MapPin,
} from "lucide-react";
...
const EMPTY_FORM: FormData = { title: "", event_date: "", location: "", image_url: "" };
...
export default function EventsPage() {
  ...
  const openEditModal = (item: Event) => {
    setFormData({ title: item.title, event_date: item.event_date, location: item.location, image_url: item.image_url });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        title: formData.title.trim(), event_date: formData.event_date,
        location: formData.location.trim(), image_url: finalImageUrl,
      };
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900 leading-tight">Agenda Event</p>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Cari event..." value={searchQuery}
        ...
        {error && (
          <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3 mb-6">
            ...
            <button onClick={fetchEvents} className="ml-auto text-red-600 underline text-xs">Coba Lagi</button>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              ...
              <p className="text-sm font-medium">{searchQuery ? "Tidak ada hasil." : "Belum ada data event."}</p>
              {!searchQuery && <button onClick={openAddModal} className="mt-4 text-green-600 text-sm font-medium hover:underline">+ Tambah event pertama</button>}
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-left">
                    ...
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Tanggal</th>
                  ...
                  {filtered.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        ...
                        <p className="text-gray-400 text-xs mt-0.5 md:hidden">{formatDate(item.event_date)}</p>
                      </td>
                      <td className="px-5 py-4 hidden md:table-cell">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-purple-50 text-purple-700">
                          <CalendarDays className="w-3 h-3" />{formatDate(item.event_date)}
                        </span>
                      </td>
                      <td className="px-5 py-4 hidden md:table-cell">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-bold text-gray-900">{isEditing ? "Edit Event" : "Tambah Event"}</h2>
              </div>
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                    placeholder="Contoh: Festival Bahari Bintan 2026" required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Tanggal Event</label>
                ...
                <div>
                  ...
                  <label htmlFor="file-event"
                    ...
                    <input id="file-event" ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={isBusy} onChange={handleFileChange} />
                  </label>
                  {imagePreview && (
