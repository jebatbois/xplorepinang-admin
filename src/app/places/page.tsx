...
import {
  Plus, Pencil, Trash2, ArrowLeft, MapPin,
...
export default function PlacesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return places.filter(
      (p) => p.name.toLowerCase().includes(q) || p.location_name.toLowerCase().includes(q)
    );
  }, [searchQuery, places]);
  ...
  const openEditModal = (item: Place) => {
    setFormData({ name: item.name, location_name: item.location_name, image_url: item.image_url });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { name: formData.name.trim(), location_name: formData.location_name.trim(), image_url: finalImageUrl };
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900 leading-tight">Rekomendasi Tempat</p>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Cari tempat..." value={searchQuery}
        ...
        {error && (
          <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3 mb-6">
            ...
            <button onClick={fetchPlaces} className="ml-auto text-red-600 underline text-xs">Coba Lagi</button>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              ...
              <p className="text-sm font-medium">{searchQuery ? "Tidak ada hasil." : "Belum ada data tempat."}</p>
              {!searchQuery && <button onClick={openAddModal} className="mt-4 text-green-600 text-sm font-medium hover:underline">+ Tambah tempat pertama</button>}
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-left">
                    ...
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Tempat</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Lokasi</th>
                  ...
                  {filtered.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        ...
                        <p className="text-gray-400 text-xs mt-0.5 md:hidden">{item.location_name || "—"}</p>
                      </td>
                      <td className="px-5 py-4 hidden md:table-cell">
                        <span className="flex items-center gap-1.5 text-gray-500 text-sm">
                          <MapPin className="w-3.5 h-3.5 text-rose-400 shrink-0" />{item.location_name || "—"}
                        </span>
                      </td>
                      <td className="px-5 py-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-bold text-gray-900">{isEditing ? "Edit Tempat" : "Tambah Tempat"}</h2>
              </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 min-h-0">
              <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                ...
                {/* Nama */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Nama Tempat <span className="text-red-500">*</span></label>
                  <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    placeholder="Contoh: Pantai Trikora" required
                ...
                {/* Lokasi */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Nama Lokasi / Area</label>
                  <input type="text" value={formData.location_name} onChange={(e) => setFormData({ ...formData, location_name: e.target.value })}
                ...
                <div>
                  ...
                  <label htmlFor="file-place"
                    ...
                    <input id="file-place" ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={isBusy} onChange={handleFileChange} />
                  </label>
                  {imagePreview && (
