...
import {
  Plus, Pencil, Trash2, ArrowLeft, UtensilsCrossed,
  X, ImageOff, Loader2, AlertCircle, Search, UploadCloud, Star,
} from "lucide-react";
...
const EMPTY_FORM: FormData = {
  ...
  rating: 4.0, opening_hours: "", image_url: "",
};
...
export default function FoodBeveragesPage() {
  ...
  const openEditModal = (item: FoodBeverage) => {
    setFormData({ name: item.name, category: item.category, description: item.description, rating: item.rating, opening_hours: item.opening_hours, image_url: item.image_url });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        description: formData.description.trim(), rating: Number(formData.rating),
        opening_hours: formData.opening_hours.trim(), image_url: finalImageUrl,
      };
      if (isEditing && editingId) {
        const { error } = await supabase.from("food_beverages").update(payload).eq("id", editingId);
        if (error) throw error;
      } else {
  ...
  const handleDelete = async (item: FoodBeverage) => {
    ...
    try {
      const { error } = await supabase.from("food_beverages").delete().eq("id", item.id);
      if (error) throw error;
      await fetchItems();
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900 leading-tight">Direktori Kuliner</p>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Cari kuliner..." value={searchQuery}
        ...
        {error && (
          <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3 mb-6">
            ...
            <button onClick={fetchItems} className="ml-auto text-red-600 underline text-xs">Coba Lagi</button>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              ...
              <p className="text-sm font-medium">{searchQuery ? "Tidak ada hasil." : "Belum ada data kuliner."}</p>
              {!searchQuery && <button onClick={openAddModal} className="mt-4 text-green-600 text-sm font-medium hover:underline">+ Tambah kuliner pertama</button>}
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-left">
                    ...
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Kategori</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Rating</th>
                    <th className="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Jam Buka</th>
                    ...
                    <tr key={item.id} className="hover:bg-gray-50/70 transition-colors">
                      <td className="px-5 py-4">
                        <div className="w-14 h-14 rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center shrink-0">
                          {item.image_url ? <Image src={item.image_url} alt={item.name} width={56} height={56} className="w-full h-full object-cover" unoptimized /> : <ImageOff className="w-5 h-5 text-gray-300" />}
                        </div>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 hidden md:table-cell">
                        <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-50 text-orange-700">{item.category}</span>
                      </td>
                      <td className="px-5 py-4 hidden md:table-cell">
                      ...
                      <td className="px-5 py-4 hidden lg:table-cell text-gray-500 text-sm">{item.opening_hours || "—"}</td>
                      <td className="px-5 py-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-bold text-gray-900">{isEditing ? "Edit Kuliner" : "Tambah Kuliner"}</h2>
              </div>
              ...
              <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    placeholder="Contoh: Warung Mak Cik Bintan" required
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" value={formData.rating} step="0.1" min="0" max="5"
                ...
                <div>
                  ...
                  <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                    placeholder="Ceritakan tentang kuliner ini..." rows={3}
                ...
                <div>
                  ...
                  <label htmlFor="file-food"
                    ...
                    <input id="file-food" ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={isBusy} onChange={handleFileChange} />
                  </label>
                  {imagePreview && (
